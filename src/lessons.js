...
const SEED = [
  ...
  // Learned from the full-portfolio visual audit (2026-07-03, 44 landings via headless Chromium):
  { scope: 'structure', rule: 'A form kit only works if its CSS is SCOPED under the kit root class (.medboxes-kit / .door-kit / .wheel-kit). An unscoped rule or a malformed selector like ".medboxes-kit /*comment*/" silently invalidates the whole following CSS block, and host CSS then overrides the kit form (grey default inputs, no dashed border, no red button). When editing a kit stylesheet, ALWAYS prefix every selector with the kit root class.' },
  { scope: 'structure', rule: 'A form kit that references image assets (url(dimg/box.png), url(images/med.png)) MUST ship those assets in its folder, otherwise copyAssets() copies nothing and the kit shows empty coloured containers. After changing a kit CSS background-image path, verify the file exists next to kit.css and gets copied into assets/_formkit-<id>/.' },
  { scope: 'structure', rule: 'When a kit shows the offer product photo (<img src=__FORMKIT_ASSET__product.png>), the injector MUST write a real product.png from the buyer uploaded offer photo (offerPhotos[0]) into the kit asset dir, or fall back to the kit own product image; otherwise the form shows a broken image icon. Never ship a kit <img> whose src resolves to a non-existent file.' },
  { scope: 'structure', rule: 'The lead form is NOT always in index.php — many landings put it in result.php / order.php / opros.php. The locator must scan every PHP/HTML file at the site root (skipping api/error/success), not just index, or the kit will be skipped with "no lead form found" while a perfectly good form sits in a sibling file.' },
  { scope: 'text', rule: 'Personal-name glossary seeding must IGNORE non-person two-capitalised-word tokens: legal/company forms (Ltd, GmbH, S.L., ИП, ООО, S.A., Inc, Corp, Co), addresses/place names (Calle L., Via R.), greetings (Dear Sir), and famous public/historical figures (Nelson Mandela, Barack Obama). Seeding the glossary with these pollutes it and produces wrong name replacements.' },
  // Learned from the Sustaflex/UZ visual bug report (2026-07-03):
  { scope: 'structure', rule: 'When injecting a form kit, the region to replace MUST cover the whole original order-form wrapper (e.g. .pachinoform, .order_block, .form-wrap), NOT just the inner <form>. Otherwise the old price/title/coupon markup stays above the kit and shows a stale "0" price floating over the boxes. The kit replaces the wrapper as one unit.' },
  { scope: 'image', rule: 'Review/testimonial photos that sit inside a .comments__item / .review / .otzyv block must NEVER be flat-replaced with the offer studio pack, even when the image classifier says product_hero. The filename heuristic (otz/review) misses numeric names like 108.jpg, so the DOM context (image inside a review container) is the authoritative signal. Such photos are edited in place (brand swap) only.' },
  { scope: 'image', rule: 'A CE/ISO conformity certificate or guarantee document that PICTURES the product package counts as product_hero, NOT text_graphic. The product itself is visible and must be swapped to the new offer. text_graphic is reserved for PURE text graphics (banner, headline, discount %, text-only certificate) with NO product photo on them.' },
  { scope: 'text', rule: 'The form-kit order price (the crossed-out old price and the new price that the injector fills from the buyer params) must NOT be localised, converted, zeroed-out or reworded by the text agent. Kit price blocks are marked translate="no" / class="notranslate" and must pass through verbatim. A free offer (new price 0) keeps its old price as the buyer set it (e.g. 990), do not force the old price to 0.' }
];
