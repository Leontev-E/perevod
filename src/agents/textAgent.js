...
b.glossary ? `NAME GLOSSARY (MANDATORY — apply exactly): whenever the source text contains (as a whole person name) any key below, you MUST render that person using the exact target name from the glossary, and you MUST use that SAME target name in EVERY fragment. Never invent a different localized name for a glossary person. Glossary ${JSON.stringify(b.glossary).slice(0, 800)}` : '',
